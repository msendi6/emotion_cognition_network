...
%
%   Note: This code just geenrate the result asscoited to Figure 2A
%   (Supplementary Data 1). The actual circulare figure will be generated in python
% code
% Requirements:
%   - MATLAB R2022b 
...
load(input_dir, 'ukbb_sfnc', 'ukbb_demo'); %Load UKBB FNC and demogrpahic 

%% Preprocess Data
missing_idx = isnan(ukbb_demo.RT);
ukbb_sfnc(missing_idx, :) = [];
ukbb_demo(missing_idx, :) = [];
score = (ukbb_demo.RT - nanmean(ukbb_demo.RT)) / nanstd(ukbb_demo.RT);
...
output_dir='~/results';
cd(output_dir)
writetable(significant_results, 'Significant_RT_Edges.csv');% This is Supplementary Data 1
