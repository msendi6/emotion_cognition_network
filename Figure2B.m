...
%   network connectivity (sFNC) and working memory (WM). It performs regression
%   analysis on sFNC edges, applies FDR correction, and outputs significant edges.
%
% Note: This code just geenrate the result asscoited to Figure 2B
% (Supplementary Data 2). The actual circulare figure will be generated in python
% code
% Requirements:
%   - MATLAB R2022b 
...
%% Preprocess Data
missing_idx = isnan(ukbb_demo.NM_max_digit);
ukbb_sfnc(missing_idx, :) = [];
ukbb_demo(missing_idx, :) = [];
score = (ukbb_demo.NM_max_digit - nanmean(ukbb_demo.NM_max_digit)) / nanstd(ukbb_demo.NM_max_digit);
...
output_dir='~/results';
cd(output_dir)
writetable(significant_results, 'Significant_WM_Edges.csv');% This is Supplementary Data 2
